...
# 各 issue ごとに git worktree を作成し、iTerm2 タブで Codex セッションを起動する。
# プロンプトは envsubst でテンプレ変数 (${ISSUE_NUMBER} 等) を実値置換した本文を
# 各 worktree 内に配置し、Codex が起動直後に読み込む設計。
#
# Usage:
...
#
# Options:
#   --prompt-template <path>   起動時に Codex が読む詳細指示テンプレ (必須・デフォルトなし)
#                              未指定はエラー。プロンプトはプロジェクト側で決めるため skill は既定値を持たない。
#                              サンプルは .agents/skills/parallel-worktree/examples/prompt-template/ を参照。
...
#
# Examples:
#   .agents/skills/parallel-worktree/scripts/spawn.sh 42:fix-typo 43:add-agent
#
# Output:
#   .agents/worktrees/<issue_number>-<slug>/                  # 各 worktree
#     ├── .session-prompt.md   (envsubst で展開済みの詳細指示)
#     └── .start-codex.sh      (iTerm 経由で実行される起動 wrapper)
#   .agents/worktrees/.issues-index.json                      # spawn 履歴
#
...
# ---------- デフォルト設定 ----------
# このスクリプトは .agents/skills/parallel-worktree/scripts/spawn.sh に配置されている前提。
...
if [[ -z "$PROMPT_TEMPLATE" ]]; then
  ...
  echo "       See .agents/skills/parallel-worktree/examples/prompt-template/README.md" >&2
  exit 2
fi
...
spawn_one() {
  ...
  # 起動スクリプト (codex に短文 kickoff prompt を引数で渡す)
  local kickoff_script="${wt_path}/.start-codex.sh"
  local kickoff_msg="このセッションは GitHub Issue #${issue_number} (${issue_title}) を実装するために起動された並列 worktree です。まず ./.session-prompt.md を読み込み、そこに書かれた手順に厳密に従って実装してください。"
  ...
  if $DRY_RUN; then
    ...
    echo "[$idx/$total] [dry-run] would open iTerm2 tab and run: cd '$wt_path' && exec ./.start-codex.sh" >&2
  else
    [[ -d "$wt_path" ]] && printf '%s' "$prompt_content" >| "$prompt_path"

    cat >| "$kickoff_script" <<KICKOFF_EOF
#!/usr/bin/env bash
# Auto-generated by .agents/skills/parallel-worktree/scripts/spawn.sh for issue #${issue_number} (ref: ${ref})
set -e
exec codex "${kickoff_msg}"
KICKOFF_EOF
...
# 既知の挙動があるため、起動コマンドは worktree 内の .start-codex.sh に固めて 1 行で呼ぶ。
open_iterm_tab() {
  local wt_path="$1"
  local cmd="cd '${wt_path}' && exec ./.start-codex.sh"

...
Phase 4 完了後、収集済みのログを用いてLLMによる品質評価を実施する。

#### 5-1. 異常スクリーニング

各テストケースの `*-quality.json`（task-breakdown-quality.json, constitution-validate-quality.json, check-spec-quality.json）を読み取り、以下を確認:

- `text_length = 0` または `section_count = 0` の場合 → **警告フラグ**（スキルが途中でブロックされた可能性）
- 警告フラグがあるスキルは、以降の評価で「出力が不完全な可能性」として注記する

#### 5-2. テキスト抽出・読み取り

以下の6つのJSONLファイルから、`type: "assistant"` のメッセージ → `message.content` 配列 → `type: "text"` のテキストを抽出して読み取る:

| # | ファイル | テストケース |
|---|---------|------------|
| 1 | `/tmp/ai-sdd-cli-ab-test/logs/sdd-workflow/task-breakdown.jsonl` | CLI無効 |
| 2 | `/tmp/ai-sdd-cli-ab-test/logs/sdd-workflow/constitution-validate.jsonl` | CLI無効 |
| 3 | `/tmp/ai-sdd-cli-ab-test/logs/sdd-workflow/check-spec.jsonl` | CLI無効 |
| 4 | `/tmp/ai-sdd-cli-ab-test/logs/sdd-workflow-with-cli/task-breakdown.jsonl` | CLI有効 |
| 5 | `/tmp/ai-sdd-cli-ab-test/logs/sdd-workflow-with-cli/constitution-validate.jsonl` | CLI有効 |
| 6 | `/tmp/ai-sdd-cli-ab-test/logs/sdd-workflow-with-cli/check-spec.jsonl` | CLI有効 |

#### 5-3. LLM評価

各スキルについて CLI無効 vs CLI有効 の出力テキストを比較し、以下の3観点で評価する:

| 評価観点 | 説明 |
|---------|------|
| **検出精度** | 仕様の矛盾・問題を正しく検出できているか |
| **レポート充実度** | 推奨アクション・修正手順が具体的か |
| **フォールバック動作** | CLI失敗時に適切にフォールバックできたか |
...
| 評価 | 意味 |
|-----|------|
| **A** | 優秀 — 高精度な検出、具体的なレポート、適切なフォールバック |
| **B** | 良好 — 概ね問題なし、軽微な改善余地あり |
| **C** | 改善必要 — 検出漏れ、レポート不足、またはフォールバック不備 |

各スキルの評価には定性コメント（改善点や特筆すべき点）を添える。
...
3. **LLM品質評価**（スキルごとの CLI無効 vs CLI有効 の A/B/C評価 + 定性コメント）

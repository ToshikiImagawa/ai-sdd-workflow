...
# sdd-workflow プラグインの成果物生成確認テスト
#
# Usage:
#   plugin-integration-test.sh setup                             - テスト環境を構築（4ケース）
#   plugin-integration-test.sh run <plugin_dir> [test_case_name]      - session-start テスト実行
#   plugin-integration-test.sh sdd-init <plugin_dir> [test_case_name] - /sdd-init テスト実行
#   plugin-integration-test.sh gen-skills <plugin_dir> [test_case_name] - 生成系スキルテスト実行
#   plugin-integration-test.sh collect <plugin_dir> [test_case_name]  - ログ収集
...
setup() {
    ...
    # テストケース: 4ケース（言語 × CLI設定）
    declare -A test_cases=(
        ["en-cli-disabled"]="en:false"
        ["en-cli-enabled"]="en:true"
        ["ja-cli-disabled"]="ja:false"
        ["ja-cli-enabled"]="ja:true"
    )

    for case_name in "${!test_cases[@]}"; do
        IFS=: read -r lang cli_enabled <<< "${test_cases[$case_name]}"
        test_dir="${TEST_BASE}/${case_name}"
...
EOF
        ...
        git commit -q -m "add .sdd-config.json (lang: ${lang}, cli.enabled: ${cli_enabled})"

        echo "テストディレクトリ作成: ${test_dir} (lang: ${lang}, cli.enabled: ${cli_enabled})"
    done
    ...
    echo "テストケース:"
    echo "  - en-cli-disabled (lang: en, cli.enabled: false)"
    echo "  - en-cli-enabled (lang: en, cli.enabled: true)"
    echo "  - ja-cli-disabled (lang: ja, cli.enabled: false)"
    echo "  - ja-cli-enabled (lang: ja, cli.enabled: true)"
}
...
_run_claude() {
    ...
    echo "$input_text" | claude --plugin-dir "$plugin_dir" --print > "$output_log" 2>&1
}
...
### en-cli-disabled (期待言語: en, CLI無効)

| テスト項目 | 結果 | 備考 |
|-----------|------|------|
| session-start.sh 実行 | - | |
| .sdd-config.json 生成 | - | |
| 言語設定 (config.json) | - | 期待値: en |
| .sdd ディレクトリ作成 | - | |
| AI-SDD-PRINCIPLES.md 配置 | - | |
| /sdd-init 前提条件 | - | session-start が正常完了していること |
| /sdd-init 実行 | - | |
...
### en-cli-enabled (期待言語: en, CLI有効)

| テスト項目 | 結果 | 備考 |
|-----------|------|------|
| session-start.sh 実行 | - | |
| .sdd-config.json 生成 | - | cli.enabled: true 設定 |
| 言語設定 (config.json) | - | 期待値: en |
| .sdd ディレクトリ作成 | - | |
| AI-SDD-PRINCIPLES.md 配置 | - | |
| /sdd-init 前提条件 | - | session-start が正常完了していること |
...
### ja-cli-disabled (期待言語: ja, CLI無効)

| テスト項目 | 結果 | 備考 |
|-----------|------|------|
| session-start.sh 実行 | - | |
| .sdd-config.json 生成 | - | |
| 言語設定 (config.json) | - | 期待値: ja |
| .sdd ディレクトリ作成 | - | |
| AI-SDD-PRINCIPLES.md 配置 | - | |
| /sdd-init 前提条件 | - | session-start が正常完了していること |
| /sdd-init 実行 | - | |
...
case "${1:-help}" in
    ...
    help|*)
        ...
        echo ""
        echo "test_case_name: オプション。テストケース名"
        echo "                例: en-cli-enabled, ja-cli-disabled"
        exit 1
        ;;
esac

...
description: "複数の GitHub Issue を並列で実装するためのスキル。各 issue を独立した git worktree + iTerm2 タブ + Claude Code セッションとして起動し、ファイル衝突なしに同時実装→PR 作成まで自走させる。ユーザーが「並列で issue 実装したい」「worktree で複数まとめて」「parallel worktree」「複数 issue を一気に」「子セッション起動」「issue を分担」と言ったとき、または独立性の高い複数 issue を効率的に処理したい場面で必ず使用する。プラグイン改修とドキュメント改修など互いに非干渉なタスクの一括処理に特に有効。"
version: 0.1.0
license: MIT
user-invocable: true
...
GitHub Issue を **並列に** 実装するためのオーケストレーションスキル。引数で指定した複数 issue それぞれを独立した git worktree + iTerm2 タブ + Claude Code 子セッションとして同時起動し、各セッションが PR 作成まで自走する仕組みを提供する。
...
- `.claude/worktrees/` ディレクトリへの書き込み権限があること
...
```bash
# 必要 CLI がそろっているか
command -v gh jq envsubst claude
...
5. `<worktree>/.start-claude.sh` 生成 (短文 kickoff prompt 付きの claude 起動 wrapper)
6. iTerm2 で新タブを作成 → `cd && exec ./.start-claude.sh` を 1 回の write text で送信
7. 起動間に 3 秒スリープ ([.git/config.lock 競合回避](https://github.com/anthropics/claude-code/issues/34645))
8. spawn 履歴を `.claude/worktrees/.issues-index.json` に追記
...
- **同時起動数**: 3〜4 が現実的。
- **`.git/config.lock` 競合**: 並列 `git worktree add` が config.lock を奪うバグ ([claude-code#34645](https://github.com/anthropics/claude-code/issues/34645))。spawn.sh は 3 秒スリープで回避済み。
- **iTerm2 必須**: AppleScript 経由で iTerm 3.x に依存。Terminal.app / Ghostty / WezTerm は未対応。
- **iTerm `write text` バッファ破壊**: 連続 2 回以上の write text は入力バッファ上で結合・破壊される既知の挙動あり。spawn.sh は起動コマンドを worktree 内 `.start-claude.sh` に固めて write text を 1 回に抑えている。

## ディレクトリ構成

```
.claude/skills/parallel-worktree/
...
└── <issue_number>-<slug>/
    ...
    ├── .start-claude.sh            ← iTerm 起動 wrapper
    └── <worktree 本体>
```
...
```bash
for wt in $(ls .claude/worktrees | grep -v '^\.'); do
  git worktree remove ".claude/worktrees/$wt" 2>/dev/null
done
git worktree prune
```
...
| 症状 | 原因 | 対処 |
|---|---|---|
| Claude が「何を進めますか？」と聞く | `.session-prompt.md` が届いていない or テンプレ変数未置換 | `cat .claude/worktrees/<name>/.session-prompt.md` で `${...}` のまま残っていないか確認。ダメなら手動で `./.session-prompt.md を Read で読み込み、その手順に従って実装してください。` を Claude に送る |
...
| `fatal: '<path>' already exists` | worktree が前回残置 | `git worktree remove --force .claude/worktrees/<name>` + `git branch -D <branch>` で完全削除 |

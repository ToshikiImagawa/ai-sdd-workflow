...
6. spec-reviewer による原則準拠チェック（必須）
   ├─ Task ツールで spec-reviewer エージェントを明示的に呼び出し
   │  例: Task(subagent_type="sdd-workflow-ja:spec-reviewer", prompt=".sdd/specification/{機能名}_spec.md")
...
7. Design Doc 生成の確認
   ...
   └─ 技術情報なし: スキップするか確認
   ↓
8. Design Doc の原則準拠チェック（design 生成時、必須）
   ├─ Task ツールで spec-reviewer エージェントを明示的に呼び出し
   │  例: Task(subagent_type="sdd-workflow-ja:spec-reviewer", prompt=".sdd/specification/{機能名}_design.md")
   ├─ CONSTITUTION.md 準拠をチェック
   ├─ spec との整合性をチェック
   ├─ 違反検出時: 自動修正を試行
   ├─ 修正後、再度チェック
...
仕様書・設計書生成後、**必ず Task ツールで `spec-reviewer` エージェントを呼び出して原則準拠をチェック**してください。

### チェックフロー

```
1. Task ツールで spec-reviewer エージェントを明示的に呼び出し
   [spec生成後]
   例: Task(
     subagent_type="sdd-workflow-ja:spec-reviewer",
     prompt=".sdd/specification/{機能名}_spec.md"
   )

   [design生成後]
   例: Task(
     subagent_type="sdd-workflow-ja:spec-reviewer",
     prompt=".sdd/specification/{機能名}_design.md"
   )
   ↓
...
5. 修正後、再度チェックして確認
   ↓
6. レビュー結果を取得し、出力に必ず含める（次のセクション参照）
```

**重要**: このチェックは省略できません。**spec生成後とdesign生成後の両方で、必ず Task ツールを使用して spec-reviewer を実行してください。**

### チェック結果の出力（必須）

**仕様書・設計書生成完了時に、以下のチェック結果セクションを必ず出力してください。このセクションは省略できません。**

```markdown
## CONSTITUTION.md 準拠チェック結果

**チェック実行日時**: YYYY-MM-DD HH:MM:SS
**レビューエージェント**: spec-reviewer

### spec チェック結果

**対象ファイル**: .sdd/specification/{機能名}_spec.md
...
**対象ファイル**: .sdd/specification/{機能名}_design.md
...
⚠️ **警告**: 仕様書・設計書にCONSTITUTION.md違反が検出されました。
上記の推奨アクションを実施し、`/generate_spec` を再実行するか、手動で修正してください。
違反を解消しないまま実装に進むと、コードレビュー時に指摘される可能性があります。
...
4. ユーザーに修正を促し、次のステップ（タスク分解・実装）に進む前に解消を推奨

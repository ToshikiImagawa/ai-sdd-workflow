...
このコマンドはAI-SDD原則に従って整合性チェックを行います。
...
**引数なしで実行された場合、処理を開始する前に対象ファイルの一覧を表示し、ユーザーに確認を求めてください。**

```markdown
## スコープ確認

引数が指定されていないため、以下の全ファイルがチェック対象となります：

| ファイル | パス |
|:--|:--|
| {機能名1}_design.md | .sdd/specification/{機能名1}_design.md |
| {機能名2}_design.md | .sdd/specification/{機能名2}_design.md |
| ... | ... |

**合計: {n}ファイル**

このスコープでチェックを実行してよろしいですか？
- 特定のファイルのみチェックする場合は、ファイル名を指定して再実行してください
- 例: `/check_spec user-auth`
```

**確認後の動作**:
- ユーザーが承認 → 全ファイルを対象にチェックを実行
- ユーザーがキャンセルまたは特定ファイルを指定 → 指定されたスコープで再実行
...
`output-templates` スキルを使用して設計書・実装整合性チェック結果を表示してください。

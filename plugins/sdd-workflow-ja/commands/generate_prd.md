...
| 引数 | 必須 | 説明 |
|:--|:--|:--|
| `要件説明` | ✅ | ビジネス要求の説明テキスト。機能名は説明から抽出されます |
...
PRD 生成後、**必ず `prd-reviewer` エージェントを呼び出して原則準拠をチェック**してください。

### チェックフロー

```
1. prd-reviewer エージェントを呼び出し
   ↓
2. CONSTITUTION.md 準拠チェックを実行
   ↓
3. 違反が検出された場合:
   ├─ 自動修正可能: Edit ツールで修正を適用
   └─ 自動修正不可能: 修正が必要な箇所をユーザーにレポートしてください
   ↓
4. 修正後、再度チェックして確認
   ↓
5. チェック結果を出力に含める
...
1. **specの更新が必要**: `/generate_spec` で再生成、またはユーザーに手動更新を推奨

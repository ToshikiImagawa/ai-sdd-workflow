...
- TaskList機能が利用できない環境では、従来通りmarkdown形式で進捗を表示してください

**TaskList エラーハンドリング**:

| エラー種別 | 対応 |
|:--|:--|
| TaskCreateツールが存在しない | markdown形式にフォールバック、ユーザーに通知 |
| タスク作成は成功したが更新に失敗 | エラーをログに記録し、実装は継続 |
| タスクIDの取得に失敗 | 依存関係設定をスキップし、警告を表示 |
| TaskUpdateで依存関係設定に失敗 | エラーを記録し、次のフェーズは手動で開始可能とする |

**検証方法**:

1. TaskCreate実行後、返却されたタスクIDを確認
2. タスクIDが空またはundefinedの場合はフォールバックモードに切り替え
3. フォールバック時はユーザーに「TaskList機能が利用できないため、markdown形式で進捗を表示します」と通知
4. 各フェーズ完了時にTaskUpdateの成功/失敗をログに記録

...
- **`recommend-front-matter`** - 既存ドキュメントにYAML front matterを追加するレコメンド機能を提供
  - すべてのAI-SDD ドキュメント（PRD、spec、design、task）のfront matter有無をスキャン
  - 推論されたメタデータ（id、title、type、status、depends-on、tags、category）を含むレコメンドを生成
  - `--apply` オプションでユーザー確認後に自動適用をサポート
  - 日英両言語のレポートテンプレートを提供
...
- **`sdd-migrate`** - レガシーマイグレーションスキルを削除（v1.x → v2.0.0 ディレクトリ構造マイグレーション）
  - 現在のユーザーにはマイグレーション機能は不要
  - メンテナンス負荷とコード複雑性を軽減

#### フック

- **`session-start.sh`** - レガシーディレクトリ構造検出と自動マイグレーションロジックを削除
  - `.sdd-config.json` 生成をデフォルト値のみに簡素化
  - lines 29-82（レガシー検出とマイグレーション警告）を削除

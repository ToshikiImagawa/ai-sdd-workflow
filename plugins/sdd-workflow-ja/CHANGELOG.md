...
- **plugin.json の skills フィールドを削除** - プラグインインストール時のエラーを修正
    - `skills` フィールドはClaude Codeの plugin.json スキーマでサポートされていないため削除
    - スキルは `skills/` ディレクトリから自動検出される仕様
    - インストール時の「Invalid input」エラーが解消
...
- **argument-hint の修正と引数説明の追加** - 全コマンドの引数仕様を実際の使用法に合わせて修正
    - `argument-hint` の表現を統一（「ファイルパス」→「機能名」に修正）
    - 各コマンドに引数説明テーブルを追加（引数名、必須/任意、説明）
    - 対象コマンド:
        - `task_breakdown`: `<設計書ファイルパス>` → `<機能名> [チケット番号]`
        - `check_spec`: `<設計書ファイルパス>` → `[機能名] [--full]`
        - `checklist`: `<ファイルパス>` → `<機能名> [チケット番号]`
        - `clarify`: `[仕様書ファイルパス]` → `<機能名> [--interactive]`
        - `constitution`: `<init|update|check>` → `<サブコマンド> [引数]`（サブコマンド詳細テーブル追加）
        - `generate_prd`: `<機能名> [要件説明]` → `<要件説明>`
        - `generate_spec`: `<機能名> [PRDファイルパス]` → `<要件説明>`
        - `implement`: `<タスクファイルパス>` → `<機能名> [チケット番号]`
...
- `session-start.sh` - 一時ファイル存在確認によるエラーハンドリング改善
    - sedコマンド失敗時の `mv: No such file or directory` エラーを修正
    - `&& [ -f "$TEMP_FILE" ]` を追加して一時ファイルの存在を確認してから mv を実行
    - フォールバック処理が適切に動作するように改善
    - 英語版に警告ファイル削除処理（else節）を追加して日本語版と統一

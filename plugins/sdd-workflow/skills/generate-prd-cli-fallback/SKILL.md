---
name: generate-prd-cli-fallback
description: "Generates complete PRD document from business requirements without CLI integration (CLI-fallback version). Creates use case diagrams, requirements analysis (UR/FR/NFR), SysML diagrams, and complete PRD file. Used when sdd-cli is not available."
...
# Generate PRD (Fallback)

Generates PRD from business requirements by orchestrating sub-skills (CLI not available fallback version).
...
```
/generate-prd-fallback A feature for users to manage tasks. Supports creation, editing, deletion.
```
...
- ✅ Skip prd-reviewer/spec-reviewer → Only perform automated checks
...
```
PRD Generation (Non-Interactive Mode):
- [ ] 1. Read prerequisites and templates
- [ ] 2. Analyze input, extract feature-name (make assumptions if needed)
- [ ] 3. Check existing PRD documents (auto-approve overwrite)
...
> **Non-Interactive Mode**: Make reasonable assumptions for ambiguous requirements. Do not ask clarifying questions.
...
**Non-Interactive Mode**: Auto-approve overwrite if PRD exists.
...
**MANDATORY**: After PRD generation, output JSON to stdout using Bash tool.

**Format specification**: See `examples/cli_fallback_json_output.md` > "PRD Generation Output"
...
cat << 'EOF'
{
  "search_results": {
    "results": [
      {
        "file_path": "/absolute/path/.sdd/requirement/feature.md",
        "file_type": "prd",
        "title": "Feature Title",
  ...
  "generated_file": "/absolute/path/.sdd/requirement/feature.md",
  "status": "success"
}
EOF
```
...
- Replace placeholders with actual values from generated PRD
- Use absolute paths (e.g., `${CLAUDE_PROJECT_DIR}/${SDD_REQUIREMENT_PATH}/...`)
- Extract values from YAML front matter of generated file
- `lint_results.issues` is empty array for CLI-fallback (no automated linting)

### Non-Interactive Mode Behavior

**All steps execute automatically without user confirmation**:

- Vibe Coding risk assessment: **Skip** (make reasonable assumptions)
- Existing PRD overwrite: **Auto-approve**
- Clarifying questions: **Skip**
- Save PRD file: **Always save**
- prd-reviewer: **Skip**
- front-matter-reviewer: **Skip**

## Post-Generation Actions

### 1. Principle Compliance

**Non-Interactive Mode**: Skip principle compliance checks (prd-reviewer and front-matter-reviewer).
